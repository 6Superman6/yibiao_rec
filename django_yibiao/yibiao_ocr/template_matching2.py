...
from Calculate2 import get_rad_val
import warnings
...
if __name__ == '__main__':
    ...
    img_and_model_list = getTemplate()
    ...
    for key_name in img_and_model_list:
        ...
        name_path = img_and_model_list[template_str]
        name = name_path.split('/')[2].split('.')[0]
        # print('name: ',name)   # name:  temp013-2
        '''原'''
        # txt_file='./img_TemplateLib_txt_2/'+name+'.txt'   # template_str[0:3]表示截取template_str（0<=下标<3）的字符串
        '''原'''
        txt_file = './img_TemplateLib_txt_3/' + name + '.txt'  # template_str[0:3]表示截取template_str（0<=下标<3）的字符串
        # print('模板对应先验信息:',txt_file)   # 模板对应先验信息: ./img_TemplateLib_txt/temp013-2.txt
        ...
        data_item = {}  # 对应模板测试图像的先验信息
        fp = open(txt_file, mode='r', encoding='utf8')  # model_type.txt是模板库文件
        lines = fp.readlines()
        for sline in lines:
            if sline != '\n':
                str_list = sline.split('-')
                ...
                type_value = str_list[1].rstrip('\n')  # 删除字符串末尾的指定字符rstrip()方法，删除末尾的\n
                # print('type_key: type_value',type_key,type_value)
                data_item[type_key] = type_value
        fp.close()
        c_x = float(data_item['c_x'])
        c_y = float(data_item['c_y'])
        a = eval(data_item['a'])    # #刻度坐标点
        # zc = int(data_item['zc'])  # 好像是说几个刻度点
        # maxd = data_item['maxd']
        maxd = float(data_item['maxd'])  # 最大刻度
        # fir_d = data_item['fir_d']
        fir_d = float(data_item['fir_d'])  # 骤然变化最小点
        # sec_d = data_item['sec_d']
        sec_d = float(data_item['sec_d'])  # 骤然变化最大点
        # scale = data_item['scale']
        scale = float(data_item['scale'])  # 一格刻度
        ...
        print('对应刻度', get_rad_val(rad[1], c_x, c_y, a, maxd, fir_d, sec_d, scale))
        print('\n**************************************************')
